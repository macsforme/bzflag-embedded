...
bool SDLDisplay::getKey(const SDL_Event& sdlEvent, BzfKeyEvent& key, bool isDown) const
{
  SDL_Keycode sym = sdlEvent.key.keysym.sym;

  // just going to get down and dirty with GCW Zero specific controls
  // some of these are ugly hacks because BZFlag is very insistent on
  // one control per keystroke, and implementing gamepad support would
  // be very invasive
  key.ascii = 0;
  ...
  case SDLK_LEFT:
	...
	// on key up, we need a standard key up so the tank stops moving
...
{
  // dirty hack to get multiple actions bound to trigger keys for observer controls
  if(event.type == SDL_KEYDOWN || event.type == SDL_KEYUP) {
    if (event.key.keysym.sym == SDLK_TAB) {
      SDL_Event fakeEvent = event;
      fakeEvent.key.keysym.sym = SDLK_F6;
      fakeEvent.key.keysym.scancode= SDL_SCANCODE_F6;
      SDL_PushEvent(&fakeEvent);
    } else if (event.key.keysym.sym == SDLK_BACKSPACE) {
      SDL_Event fakeEvent = event;
      fakeEvent.key.keysym.sym = SDLK_F7;
      fakeEvent.key.keysym.scancode= SDL_SCANCODE_F7;
      SDL_PushEvent(&fakeEvent);
    } else if (event.key.keysym.sym == SDLK_LSHIFT) {
      SDL_Event fakeEvent = event;
      fakeEvent.key.keysym.sym = SDLK_F8;
      fakeEvent.key.keysym.scancode= SDL_SCANCODE_F8;
      SDL_PushEvent(&fakeEvent);
    }
  }
  ...
  case SDL_KEYDOWN:
    _event.type = BzfEvent::KeyDown;
    if (!getKey(event, _event.keyDown, true))
      return false;
    break;

  case SDL_KEYUP:
    _event.type = BzfEvent::KeyUp;
    if (!getKey(event, _event.keyUp, false))
      return false;
    break;
